...
| 27 | B1TCMPCEN | RW | - | B1TCM ECC 檢查致能（reset 依 PARECCENRAMm[2] 接腳；審查轉錄） |
| 26 | B0TCMPCEN | RW | - | B0TCM ECC 檢查致能（reset 依 PARECCENRAMm[1] 接腳；審查轉錄） |
| 25 | ATCMPCEN | RW | - | ATCM ECC 檢查致能（reset 依 PARECCENRAMm[0] 接腳；審查轉錄） |
...
| 2 | B1TCMECEN | RW | - | B1TCM 外部錯誤致能（reset 依 ERRENRAMm[2] 接腳；審查轉錄） |
| 1 | B0TCMECEN | RW | - | B0TCM 外部錯誤致能（reset 依 ERRENRAMm[1] 接腳；審查轉錄） |
| 0 | ATCMECEN | RW | - | ATCM 外部錯誤致能（reset 依 ERRENRAMm[0] 接腳；審查轉錄） |
...
- Offset: 0x078
- Reset: -
- Description: Auxiliary Instruction Fault Status Register — 指令側同位／ECC 錯誤的補充資訊（來源與可否恢復；CacheWay 在 AIFSR 上為 Unpredictable、Index 為 SBZ；MRC/MCR p15,0,Rt,c5,c1,1）。整表依交叉審查轉錄自 DDI 0460D Figure 4-33／Table 4-31/4-32，尚未親驗原文。**有效性（Table 4-31 註腳）**：僅當 IFSR 回報同位/ECC 錯誤時，本暫存器內容才有效——其他時間讀值不可預期（Unpredictable）

| Bits  | Field | Access | Reset | Description |
|-------|-------|--------|-------|-------------|
| 31:28 | RESERVED | RO | - | 保留（Table 4-31 僅標 SBZ——寫入須為零，無讀值／reset 依據；審查轉錄，R13 自 RES0/0 回退） |
| 27:24 | CacheWay | RW | - | Cache way——Table 4-31 註腳：僅 data cache store 同位/ECC 錯誤時有效；AIFSR（指令側）不存在此情境，因此本欄讀值為 Unpredictable，不得解讀（審查轉錄） |
| 23:22 | Side | RW | - | 錯誤來源側低 2 位，與 SideExt 併讀（Table 4-32；審查轉錄）：SideExt:Side＝0:00 Cache/AXIM、0:01 ATCM、0:10 BTCM、1:01 AXI 周邊埠（含虛擬介面）、1:10 AHB 周邊埠、0:11/1:00/1:11 保留 |
| 21 | Recoverable | RW | - | 錯誤可否恢復（審查轉錄） |
| 20 | SideExt | RW | - | 錯誤來源側擴充位（高位；八組組合表見 Side 欄；審查轉錄） |
| 19:14 | RESERVED | RO | - | 保留（Table 4-31 僅標 SBZ——寫入須為零，無讀值／reset 依據；審查轉錄，R13 自 RES0/0 回退） |
