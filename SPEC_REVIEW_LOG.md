...
## 一、已套用的決議（#1–26＝R3R；#27–33＝R4R；#34–40＝R5R；#41–47＝R6R；#48–50＝R7R；#51–53＝R8R；#54–56＝R9R；#57–59＝R10R；#60–61＝R11R；#62–63＝R12R；#64–68＝R13R；#69–71＝FinalR；#72＝FinalR 複驗 doc-only）
...
| 65 | 本檔 | **AMENDED by #71**（解鎖路徑放寬——官方 PDF 不強制 commit 入 repo：可讀官方 URL＋版次＋SHA-256／使用者附件／審查環境可讀本地副本皆可；Verified 升級仍以審查環境實際可讀為前提）——官方文件通道狀態更新（R13-02）：Codex 2026-08-31 實測 developer.arm.com／documentation-service.arm.com 已可下載——DDI 0460D（download `https://documentation-service.arm.com/static/5f04288cdbdee951c1cd8969`、檔名 DDI0460D_cortex_r5_r1p2_trm.pdf、3,268,393 bytes、468 頁、SHA-256 `da00857f6e3d429354e8606bc1de2012782389dc3da0be829a19f0f680c2188b`）與 100442_0200_02_en（download `https://documentation-service.arm.com/static/649ac6d4df6cd61d528c2bf1`、4,680,444 bytes）——以上數值為 Codex 回報之審查轉錄。我方沙箱同日重試：代理對 documentation-service.arm.com／infocenter.arm.com CONNECT 回 403 host_not_allowed（出口 allowlist 政策，非 ARM 端問題）。**解鎖路徑：使用者將官方 PDF 放入 repo（或開放網域）即可親驗升級** | 審查轉錄＋親驗（我方 403 實測） |
...
| 72 | 本檔 | 文件一致性收尾（結案複驗唯一殘留，doc-only）：#65 補標 AMENDED by #71、第二節解鎖指引同步為 #71 規則（審查環境可讀的一手證據即可，官方 PDF 不強制進 repo）——消除同帳並存的新舊兩套指引；標記測試加 #65→#71 斷言＋第二節舊句全檔禁入。不動 spec／程式、不重啟內容審查 | 慣例（文件一致性） |

## 二、待原文親驗後回填（官方 PDF 公網已可取得——見 #65）

文件狀態（2026-08-31）：**DDI 0460D** 與 **100442_0200_02_en** 的官方下載端點已恢復可達
（Codex 實測，URL/雜湊見 #65）；我方審查環境當時仍受 arm.com 出口政策限制（403
host_not_allowed，同日重試實測）。**升級 Verified 的條件（#71）：審查環境必須能實際讀取
對應官方 PDF**——文件可由可讀的官方 URL、使用者附件或審查環境可讀的本地副本提供，
**不要求 commit 進 repo**；repo 僅保存來源 URL、版次、SHA-256、頁碼與驗證結論。屆時
逐顆、逐表核對後把「審查轉錄」升級為 Verified 並回填下列值（不得批次升級）。
**N25/N45 datasheet 或 AndeStar V5 SPA** 仍待取得。
